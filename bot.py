...
# ================== КОНСТАНТЫ ДИЗАЙНА ==================
COLORS = {
    'up': '#2ecc71',      # Зеленый для роста
    'down': '#e74c3c',    # Красный для падения
    'wick_up': '#27ae60', # Темно-зеленый для верхних теней
    'wick_down': '#c0392b', # Темно-красный для нижних теней
    'ema_fast': '#3498db', # Голубой для быстрой EMA
    'ema_slow': '#9b59b6', # Фиолетовый для медленной EMA
    'supertrend': '#f39c12', # Оранжевый для Supertrend
    'grid': '#ededed',    # Цвет сетки
    'text': '#2c3e50',    # Основной цвет текста
...
def create_candlestick_plot(df: pd.DataFrame, indicators: Dict[str, Any]) -> io.BytesIO:
    try:
        # Стилизованные цвета
        ...
        fig, _ = mpf.plot(
            ...
            figscale=1.1,
            figratio=(10, 6),
            title=f"\n\n{user_data.get('symbol', '')} ({user_data.get('timeframe', '')})",
            returnfig=True
        )
...
def process_timeframe_selection(update, context, user_data, user_id, chat_id, tf_idx):
    ...
    try:
        ...
        # Оптимизация параметров для этой пары и таймфрейма
        optimized_settings = optimize_parameters(symbol, timeframe, user_id)
        ...
        # Проверка мультитаймфреймного подтверждения
        signal, emoji, strength = generate_signal(df['close'].values, indicators)
        if "BUY" in signal:
            if not check_multi_timeframe_confirmation(symbol, user_id, "BUY"):
                signal = "Неподтвержденный " + signal
                strength = max(0, strength - 1)
        elif "SELL" in signal:
            if not check_multi_timeframe_confirmation(symbol, user_id, "SELL"):
                signal = "Неподтвержденный " + signal
                strength = max(0, strength - 1)
...
def send_analysis_results(context, chat_id, user_id, user_data, df, timeframe, candle_plot, tech_plot, signal, emoji, strength):
    # Форматированное сообщение (без строки загрузки индикаторов)
    text = f"""
...
"""
    ...
    # Отправка графика индикаторов с сигналом
    try:
