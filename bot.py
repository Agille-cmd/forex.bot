...
def calculate_indicators(df: pd.DataFrame, user_id: int) -> Optional[Dict[str, Any]]:
    try:
        ...
        hl2 = (high + low) / 2
        supertrend_upper = hl2 + (settings['SUPERTREND_MULTIPLIER'] * atr)
        supertrend_lower = hl2 - (settings['SUPERTREND_MULTIPLIER'] * atr)
        
        supertrend = np.zeros_like(close)
        trend = 1  # 1 = uptrend, -1 = downtrend
        
        for i in range(1, len(close)):
            if close[i] > supertrend_upper[i-1]:
                trend = 1
            elif close[i] < supertrend_lower[i-1]:
                trend = -1
...
def create_technical_plot(df: pd.DataFrame, indicators: Dict[str, Any]) -> io.BytesIO:
    try:
        ...
        if 'supertrend' in indicators:
            ax1.plot(df.index, indicators['supertrend'], label='Supertrend', 
                    color='green' if indicators['supertrend_trend'] == 1 else 'red', linewidth=1.5)
...
def button_handler(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        elif query.data == 'set_supertrend':
            user_data = ensure_user_data(user_id)
            user_data['step'] = 'set_supertrend'
            safe_edit_message(
                context,
                chat_id,
                message_id,
                "Введите параметры Supertrend в формате PERIOD/MULTIPLIER (например 10/3.0):",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔙 Назад", callback_data='settings_menu')]
                ])
            )
...
def show_settings_menu(update: Update, context: CallbackContext, user_id: int) -> None:
    try:
        ...
        buttons = [
            ...
            (f"Supertrend: {settings['SUPERTREND_PERIOD']}/{settings['SUPERTREND_MULTIPLIER']}", "set_supertrend"),
...
def handle_settings_input(update: Update, context: CallbackContext, user_id: int, text: str) -> None:
    try:
        ...
        elif current_step == 'set_supertrend':
            try:
                parts = text.split('/')
                if len(parts) == 2:
                    period = int(parts[0])
                    ...
                    if 5 <= period <= 30 and 1.0 <= multiplier <= 5.0:
                        ...
                        msg_id = safe_send_message(
                            context,
                            chat_id,
                            f"✅ Параметры Supertrend изменены на {period}/{multiplier}",
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton("🔙 Назад к настройкам", callback_data='settings_menu')]
                            ])
                        )
